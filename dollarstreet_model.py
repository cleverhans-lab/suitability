...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        wandb.log(
            {"epoch": epoch, "train_loss": epoch_loss, "train_accuracy": epoch_acc}
        )
...
def main(args):
    ...
    train_loader = get_dollarstreet(
        args.root_dir, "train", args.batch_size, shuffle=True, num_workers=4
    )  # pre_filter=lambda x: x['region.id'] == 'eu'
    test_loader = get_dollarstreet(
        args.root_dir, "test", args.batch_size, shuffle=False, num_workers=4
    )  # pre_filter=lambda x: x['region.id'] == 'eu'
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train ResNet50 on DollarStreet and log metrics to wandb"
    )
    parser.add_argument(
        "--root_dir", type=str, required=True, help="Root directory for the dataset"
    )
    parser.add_argument(
        "--log_dir",
        type=str,
